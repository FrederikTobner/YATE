...
void editor_initialize(configuration_reader_result_t *config) {
  editorConfig.cursorCurrentX = editorConfig.renderX =
      editorConfig.cursorCurrentY = editorConfig.rowOffset =
          editorConfig.columnOffset = editorConfig.numberOfRows =
              editorConfig.statusMessageTimeStamp = 0;
  ...
  if (editor_get_window_size(&editorConfig.screenRows,
                             &editorConfig.screenColumns) == -1) {
  ...
  case PAGE_DOWN: {
    if (c == PAGE_UP) {
      ...
      editorConfig.cursorCurrentY =
          editorConfig.rowOffset + editorConfig.screenRows - 1;
      if (editorConfig.cursorCurrentY > editorConfig.numberOfRows)
...
void editor_refresh_screen() {
  ...
  snprintf(buf, sizeof(buf), "\x1b[%d;%dH",
           (editorConfig.cursorCurrentY - editorConfig.rowOffset) + 1,
...
static void editor_delete_character() {
  ...
  if (editorConfig.cursorCurrentX > 0) {
    ...
    editorConfig.cursorCurrentX =
        editorConfig.editorRows[editorConfig.cursorCurrentY - 1].size;
    editor_row_append_string(
        &editorConfig.editorRows[editorConfig.cursorCurrentY - 1], row->chars,
        row->size);
    editor_delete_row(editorConfig.cursorCurrentY);
    editorConfig.cursorCurrentY--;
  }
...
static void editor_draw_message_bar(append_buffer_t *buffer) {
  ...
  if (msglen && time(NULL) - editorConfig.statusMessageTimeStamp <
                    editorConfig.config->messageDisplayDuration)
...
static void editor_render_welcome_screen_row(append_buffer_t *buffer,
                                             uint32_t rowIndex) {
  ...
  case 0:
    welcomeMessageRowLength =
        snprintf(welcomeMessageRow, sizeof(welcomeMessageRow),
                 "%s - Yet another text editor", PROJECT_NAME);
    break;
  case 1:
    return;
  case 2:
    welcomeMessageRowLength =
        snprintf(welcomeMessageRow, sizeof(welcomeMessageRow), "version %d.%d",
                 PROJECT_VERSION_MAJOR, PROJECT_VERSION_MINOR);
    break;
  case 3:
    welcomeMessageRowLength = snprintf(
        welcomeMessageRow, sizeof(welcomeMessageRow), "by %s", PROJECT_VENDOR);
    break;
  case 4:
    welcomeMessageRowLength =
        snprintf(welcomeMessageRow, sizeof(welcomeMessageRow),
  ...
  case 7:
    welcomeMessageRowLength = snprintf(
        welcomeMessageRow, sizeof(welcomeMessageRow),
  ...
  while (padding--)
    append_buffer_append_string(buffer, " ", 1);
  append_buffer_append_string(buffer, welcomeMessageRow,
                              welcomeMessageRowLength);
}
  ...
  for (y = 0; y < editorConfig.screenRows; y++) {
    ...
    if (filerow >= editorConfig.numberOfRows) {
      if (editorConfig.numberOfRows == 0 && y >= editorConfig.screenRows / 3 &&
        ...
        editor_render_welcome_screen_row(buffer,
                                         y - editorConfig.screenRows / 3);
      else
        append_buffer_append_string(buffer, "~", 1);
    } else {
      uint32_t length = editorConfig.editorRows[filerow].renderSize -
                        editorConfig.columnOffset;
      if (length < 0)
        length = 0;
      if (length > editorConfig.screenColumns)
        length = editorConfig.screenColumns;
      char *c =
          &editorConfig.editorRows[filerow].render[editorConfig.columnOffset];
      unsigned char *highLight = &editorConfig.editorRows[filerow]
                                      .highLight[editorConfig.columnOffset];
        ...
        if (iscntrl(c[j])) {
          ...
          if (color != current_color) {
            ...
            int clen = snprintf(buf, sizeof(buf),
                                (color & 0xff000000) ? "\x1b[48;2;%d;%d;%dm"
                                                     : "\x1b[38;2;%d;%d;%dm",
                                (color & 0x00ff0000) >> 16,
...
static void editor_draw_status_bar(append_buffer_t *buffer) {
  ...
  int leftMessageLength = snprintf(
      statusBarLeftMessage, sizeof(statusBarLeftMessage), "%.80s - %d lines %s",
      editorConfig.fileName ? realPath : "[No file name]",
      editorConfig.numberOfRows,
      editorConfig.unsavedChanges ? "(modified)" : "");
  int rightMessageLength = snprintf(
      StatusBarRightMessage, sizeof(StatusBarRightMessage), "%s | %d/%d",
  ...
  while (leftMessageLength < editorConfig.screenColumns) {
    if (editorConfig.screenColumns - leftMessageLength == rightMessageLength) {
      append_buffer_append_string(buffer, StatusBarRightMessage,
                                  rightMessageLength);
      break;
    } else {
      append_buffer_append_string(buffer, " ", 1);
...
static void editor_execute() {
  ...
  if (!strcmp(editorConfig.syntax->filetype, "Cellox")) {
    ...
    editor_set_status_message("Executing %s files is not supported",
                              editorConfig.syntax->filetype);
    return;
  }
...
static void editor_find_callback(char *query, uint32_t key) {
  ...
  if (savedHighLight) {
    memcpy(editorConfig.editorRows[savedHighLightedLine].highLight,
           savedHighLight,
  ...
  for (i = 0; i < editorConfig.numberOfRows; i++) {
    ...
    if (match) {
      ...
      editorConfig.cursorCurrentX =
          editor_row_rx_to_cx(row, match - row->render) + 1;
      ...
      memset(&row->highLight[match - row->render], HIGHLIGHT_MATCH,
             strlen(query));
      break;
    }
  }
...
static void editor_insert_character(uint32_t c) {
  ...
  editor_row_insert_character(
      &editorConfig.editorRows[editorConfig.cursorCurrentY],
      editorConfig.cursorCurrentX, c);
  editorConfig.cursorCurrentX++;
}
...
static void editor_insert_newline() {
  if (editorConfig.cursorCurrentX == 0) {
    ...
    editor_row_t *row = &editorConfig.editorRows[editorConfig.cursorCurrentY];
    editor_insert_row(editorConfig.cursorCurrentY + 1,
                      &row->chars[editorConfig.cursorCurrentX],
...
static void editor_insert_row(uint32_t at, char *str, size_t length) {
  if (at < 0 || at > editorConfig.numberOfRows)
    return;
  editorConfig.editorRows =
      realloc(editorConfig.editorRows,
              sizeof(editor_row_t) * (editorConfig.numberOfRows + 1));
...
static void editor_open_file() {
  ...
  uint32_t savedRowOffset = editorConfig.rowOffset;
  char *query =
      editor_prompt("Open: %s (Use ESC/Enter)", editor_open_file_callback);
  if (query) {
    free(query);
  } else {
    editorConfig.cursorCurrentX = savedCurrentX;
...
static inline void editor_paste_line() {
  if (editorConfig.copyBuffer.buffer && editorConfig.copyBuffer.length)
    editor_insert_row(editorConfig.cursorCurrentY,
                      editorConfig.copyBuffer.buffer,
                      editorConfig.copyBuffer.length);
}
...
static void editor_row_append_string(editor_row_t *row, char *str,
                                     size_t length) {
...
static uint32_t editor_row_cx_to_rx(editor_row_t *row,
                                    uint32_t cursorCurrentX) {
  uint32_t renderX = 0;
  uint32_t j;
  for (j = 0; j < cursorCurrentX; j++) {
    if (row->chars[j] == '\t')
      renderX += (editorConfig.config->tabStopSize - 1) -
                 (renderX % editorConfig.config->tabStopSize);
    renderX++;
  }
  return renderX;
}
...
/// @param c The character that is inserted
static void editor_row_insert_character(editor_row_t *row, uint32_t at,
                                        uint32_t c) {
  if (at < 0 || at > row->size)
    at = row->size;
...
static uint32_t editor_row_rx_to_cx(editor_row_t *row, uint32_t renderX) {
  ...
  for (cursorCurrentX = 0; cursorCurrentX < row->size; cursorCurrentX++) {
    if (row->chars[cursorCurrentX] == '\t')
      cur_rx += (editorConfig.config->tabStopSize - 1) -
                (cur_rx % editorConfig.config->tabStopSize);
    cur_rx++;
    if (cur_rx > renderX)
...
static char *editor_rows_to_string(uint32_t *bufferLength) {
  ...
  *bufferLength = totlen;
  ...
  for (j = 0; j < editorConfig.numberOfRows; j++) {
    memcpy(bufferPointer, editorConfig.editorRows[j].chars,
           editorConfig.editorRows[j].size);
...
static void editor_scroll() {
  ...
  if (editorConfig.cursorCurrentY < editorConfig.numberOfRows) {
    editorConfig.renderX = editor_row_cx_to_rx(
        &editorConfig.editorRows[editorConfig.cursorCurrentY],
        editorConfig.cursorCurrentX);
  }
  ...
  if (editorConfig.cursorCurrentY >=
      editorConfig.rowOffset + editorConfig.screenRows) {
    editorConfig.rowOffset =
        editorConfig.cursorCurrentY - editorConfig.screenRows + 1;
  }
  if (editorConfig.renderX < editorConfig.columnOffset) {
    editorConfig.columnOffset = editorConfig.renderX;
  }
  if (editorConfig.renderX >=
      editorConfig.columnOffset + editorConfig.screenColumns) {
    editorConfig.columnOffset =
        editorConfig.renderX - editorConfig.screenColumns + 1;
  }
}
  ...
  for (size_t j = 0; j < languageCount; j++) {
    ...
    while (s->filematch[i]) {
      ...
      if ((isFileExtension && fileExtension &&
           !strcmp(fileExtension, s->filematch[i])) ||
          (!isFileExtension &&
           strstr(editorConfig.fileName, s->filematch[i]))) {
        editorConfig.syntax = s;
        for (uint32_t filerow = 0; filerow < editorConfig.numberOfRows;
             filerow++)
...
static void editor_set_status_message(char const *format, ...) {
  ...
  vsnprintf(editorConfig.statusMessage, sizeof(editorConfig.statusMessage),
            format, argumentPointer);
...
static inline void editor_show_help() {
  editor_set_status_message(
      "HELP: Ctrl-D = delete | Ctrl-F = find | Ctrl-H = help | Ctrl-O = open | "
...
static void editor_update_row(editor_row_t *row) {
  ...
  free(row->render);
  row->render =
      malloc(row->size + tabs * (editorConfig.config->tabStopSize - 1) + 1);
  size_t idx = 0;
  for (j = 0; j < row->size; j++) {
    if (row->chars[j] == '\t') {
      row->render[idx++] = ' ';
...
static void editor_update_syntax(editor_row_t *row) {
  ...
  size_t singleLineCommentStartLength =
      singleLineCommentStart ? strlen(singleLineCommentStart) : 0;
  size_t multiLineCommentStartLength =
      multiLineCommentStart ? strlen(multiLineCommentStart) : 0;
  size_t multiLineCommentEndLength =
      multiLineCommentEnd ? strlen(multiLineCommentEnd) : 0;
  bool previousSeperator = true;
  int insideString = 0;
  bool insideComment =
      (row->index > 0 &&
       editorConfig.editorRows[row->index - 1].hightLightOpenComment);
  size_t i = 0;
  while (i < row->renderSize) {
    char c = row->render[i];
    unsigned char prevoiusHighlighting =
        (i > 0) ? row->highLight[i - 1] : HIGHTLIGHT_NORMAL;

    if (singleLineCommentStartLength && !insideString && !insideComment) {
      if (!strncmp(&row->render[i], singleLineCommentStart,
                   singleLineCommentStartLength)) {
        memset(&row->highLight[i], HIGHLIGHT_COMMENT, row->renderSize - i);
        break;
      }
    }
    if (multiLineCommentStartLength && multiLineCommentEndLength &&
        !insideString) {
      if (insideComment) {
        row->highLight[i] = HIGHLIGHT_MLCOMMENT;
        if (!strncmp(&row->render[i], multiLineCommentEnd,
                     multiLineCommentEndLength)) {
          memset(&row->highLight[i], HIGHLIGHT_MLCOMMENT,
                 multiLineCommentEndLength);
      ...
      } else if (!strncmp(&row->render[i], multiLineCommentStart,
                          multiLineCommentStartLength)) {
        memset(&row->highLight[i], HIGHLIGHT_MLCOMMENT,
               multiLineCommentStartLength);
    ...
    if (editorConfig.syntax->flags & SYNTAX_HIGHLIGHT_NUMBERS) {
      if ((isdigit(c) &&
           (previousSeperator || prevoiusHighlighting == HIGHLIGHT_NUMBER)) ||
    ...
    if (previousSeperator) {
      ...
      for (j = 0; keywords[j]; j++) {
        ...
        bool belongsToSecondKeywordGroup =
            keywords[j][keywordLength - 1] == '|';
        bool belongsToThirdKeywordGroup = keywords[j][keywordLength - 1] == '&';
        bool belongsToFourthKeywordGroup =
            keywords[j][keywordLength - 1] == '~';
        if (belongsToSecondKeywordGroup || belongsToThirdKeywordGroup ||
            belongsToFourthKeywordGroup)
          keywordLength--;
        if (!strncmp(&row->render[i], keywords[j], keywordLength) &&
          ...
          if (belongsToSecondKeywordGroup)
            memset(&row->highLight[i], HIGHLIGHT_KEYWORDS_SECOND_GROUP,
                   keywordLength);
          else if (belongsToThirdKeywordGroup)
            memset(&row->highLight[i], HIGHLIGHT_KEYWORDS_THIRD_GROUP,
                   keywordLength);
          else if (belongsToFourthKeywordGroup)
            memset(&row->highLight[i], HIGHLIGHT_KEYWORDS_FOURTH_GROUP,
                   keywordLength);
          else
            memset(&row->highLight[i], HIGHLIGHT_KEYWORDS_FIRST_GROUP,
                   keywordLength);
          i += keywordLength;
          break;
        }
