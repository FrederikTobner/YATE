 ...
 * @brief File containing the implementation of the configuration reader and the
...
static int configuration_reader_parse_configuration_file_line(
    char **, configuration_reader_result_t **);
...
configuration_reader_result_t *configuration_reader_read_configuration_file() {
  configuration_reader_result_t *result =
      malloc(sizeof(configuration_reader_result_t));
...
/// @param fileContentPointer Pointer to the pointer that points to the
/// character sequence representing the configuration file content
/// @param result The result of the config file reading process
/// @return 0 if the current line is the last line in the configuration file,
/// otherwise 1
static int configuration_reader_parse_configuration_file_line(
    char **fileContentPointer, configuration_reader_result_t **result) {
  ...
  memccpy(optionRaw,
          (*fileContentPointer) - optionRawLength - argumentRawLength - 1,
          sizeof(char), optionRawLength);
  optionRaw[optionRawLength] = '\0';
  argumentRaw[argumentRawLength] = '\0';
  memccpy(argumentRaw, (*fileContentPointer) - argumentRawLength, sizeof(char),
          argumentRawLength);
...
/// @brief Removes all the whitespace characters from the beginning and the end
/// of a character sequence
/// @param str The character sequence where the whitespace characters are
/// removed
